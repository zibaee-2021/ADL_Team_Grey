...
def view_batch(data_loader):
    ...
    for batch in data_loader:
        ...
        # Convert to Numpy and rescale images from [0, 1] to [0, 255] and convert to uint8
        images_np = images.numpy()
        images_np = (images_np * 255).astype(np.uint8)
        # Plot each image in the batch
        for i in range(images.size(0)):
            plt.imshow(images_np[i].transpose(1, 2, 0))  # Matplotlib expects image in (H, W, C) format
            plt.show()
        break  # Exit th
...
def one_hot_to_img(one_hot):
    """
    Converts the one-hot segmented image produced by the model into a numpy array
    """
    one_hot_2d = one_hot.view(one_hot.size(0) * one_hot.size(2) * one_hot.size(3), -1) #take out the hot dimension
    gray_images = torch.argmax(one_hot_2d, dim=1).float() # extract the hot index
    gray_images /= one_hot.size(1)-1 # normalise to [0,1]
    gray_images = gray_images.view(one_hot.size(0), one_hot.size(2), one_hot.size(3))
    gray_images_np = (gray_images.cpu().numpy()*255).astype(np.uint8)

    return gray_images_np
...
class OxfordPetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        segment_labels[0][trimap_tensor[0] == 1] = 1 #foregrount
        segment_labels[1][trimap_tensor[0] == 2] = 1 #background
        segment_labels[2][trimap_tensor[0] == 3] = 1 #boundary
    ...
    def split_dataset(self, train_split, val_split, test_split, batch_size):
        ...
        val_loader = DataLoader(self, batch_size = 1, sampler=val_sampler)
...
if __name__ == '__main__':
    ...
    if check_semantic_segmentation:
        ...
        for batch in test_loader:
            ...
            output_labels = one_hot_to_img(outputs)
