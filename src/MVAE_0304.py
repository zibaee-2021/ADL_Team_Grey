...
def view_batch(data_loader, model):
    ...
    plt.imshow(one_hot_to_tensor(test_output)[0].cpu())
...
def view_training(model, loader, display):
    for batch in loader:
        ...
        if display is False:
            plt.ioff()
        fig, axes = plt.subplots(4, images.size(0), figsize=(12,8))
        for i in range(images.size(0)):
            if images.size(0)>1:
                ax0 = axes[0,i]
                ax1 = axes[1,i]
            ...
            ax0.axis('off')
            ax0.set_title('Image')
            ax0.imshow(images[i].cpu().permute(1,2,0))
            ax1.axis('off')
            ax1.set_title('Label')
            ax1.imshow(torch.argmax(labels[i],dim=0)/2.0)
            ax2.axis('off')
            ax2.set_title('Output (prob)')
            ax2.imshow(outputs.detach().cpu()[i].permute(1,2,0))
            ax3.axis('off')
            ax3.set_title('Output (argmax)')
            ax3.imshow(output_labels[i]/2.0)
        plt.tight_layout()
        if display:
            plt.show()
...
def check_random_image(model, parameters):
    num_channels = parameters['num_channels']
    image_size = parameters['image_size']
    random_image = torch.rand(num_channels,image_size,image_size).to(device)
    random_image = random_image.unsqueeze(0) # create batch dimension
    segment_map = one_hot_to_tensor(model(random_image))
    fig, axes = plt.subplots(1,2)

    axes[0].axis('off')
    axes[0].imshow(random_image[0].cpu().permute(1,2,0))
    axes[1].axis('off')
    axes[1].imshow(segment_map[0].cpu().unsqueeze(0).permute(1,2,0))
    plt.tight_layout()
    plt.show()

def plt_img_lbl_out(img, lbl, out):
    ...
    axes[2].set_title('output (prob)')
    axes[3].set_title('output (argmax)')
    axes[0].imshow(img)
    axes[1].imshow(lbl)
    axes[2].imshow(out)
    axes[3].imshow(torch.argmax(out, dim=2))
    plt.tight_layout()
    plt.show()
...
class VisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        #self.vit = models.vision_transformer.vit_b_16()
...
class ViTDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.quarter_image_size = int(self.image_size/4)
        self.fc=nn.Linear(self.input_dim, self.image_size*self.image_size*self.num_channels)
        self.relu = nn.ReLU()

    def forward(self, x):
        bs=x.size(0)
        #x = torch.exp(x) # expands the differences between x values, which are otherwise very close
        x = self.relu(self.fc(x))
        x = x.view(bs,-1,self.image_size,self.image_size)
        #x = F.interpolate(x, scale_factor=4, mode='nearest')
        #x = torch.softmax(x, dim=1) # convert to probabilities over class dimension
        return x
...
class VisionTransformerDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=self.upscale, mode='nearest') # Output: B * 8 * 112 * 112
...
class Linear(nn.Module):
    def __init__(self, params):
        ...
        self.image_size = params['image_size']
        self.num_channels = params['num_channels']
        self.fc= nn.Linear(self.num_channels*self.image_size*self.image_size, self.input_dim)
        self.relu = nn.ReLU()
...
class LinearDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.image_size = params['image_size']
        self.num_channels = params['num_channels']
        self.fc= nn.Linear(self.input_dim, self.num_channels*self.image_size*self.image_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        bs = x.size(0)
        x = self.relu(self.fc(x))
        x = x.view(bs,self.num_channels,self.image_size,self.image_size)
        x = self.softmax(x)
        return x
...
class OxfordPetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_transform = transforms.Compose([transforms.Resize((self.image_size, self.image_size)), # Convert image to tensor # Scale pixel values to [0, 255] and convert to uint8
                                              transforms.ToTensor(),
...
class SegmentationClassifier(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.fc1 = nn.Linear(self.num_features,3072)
        self.fc2 = nn.Linear(3072,self.image_size*self.image_size*self.num_channels) #9408
        self.relu = nn.ReLU()
        self.activation = nn.Tanh() # squashes output
...
if __name__ == '__main__':
    ...
    params = {
        ...
        "patch_size": 2,               # must be divisor of image_size
        # vision transformer encoder
        "vit_num_features": 20,        # 768 number of features created by the vision transformer
        "vit_num_layers": 4,            # 12ViT parameter
        "vit_num_heads": 4,             # 8 ViT parameter
        "vit_hidden_dim": 128,          # 512 ViT parameter
        "vit_mlp_dim": 256,             # 1024 ViT parameter
        ...
        for epoch in range(pt_num_epochs):
            ...
            for its, (input_images, input_labels) in enumerate(dataloader):
                ...
                if its % report_every == (report_every-1):    # print every report_every mini-batches
                    print('Epoch [%d / %d],  %d image minibatch [%4d / %4d], running loss: %.4f' % (epoch + 1, pt_num_epochs, pt_batch_size, its + 1, len(dataloader) ,running_loss / len(dataloader)))
            scheduler.step()
            epoch_end_time=time.time()
            print(f"Epoch [{epoch + 1}/{pt_num_epochs}] completed in {(epoch_end_time-epoch_start_time):.0f}s, Loss: {running_loss / len(dataloader):.4f}")
    ...
    if check_infilling:
        ...
        with torch.no_grad():
            ...
            infill_image_tensor = infill_image_tensor.reshape(params['num_channels'], params['image_size'], params['image_size'])
            inpainted_image_tensor = masked_image_tensor[0].to(device) + infill_image_tensor.to(device) * (masked_image_tensor[0].to(device) == 0).float()
    ...
    #segmentation_decoder = SegmentationClassifier(params)
    # segmentation_decoder = ViTDecoder(params)
    segmentation_decoder = LinearDecoder(params)
    ft_segmentation_model = SemanticSegmenter(encoder, segmentation_decoder).to(device)
    initialise_weights(encoder)
    initialise_weights(segmentation_decoder)

    if load_models and os.path.isfile(script_dir+segmentation_decoder_file):
        print("Loading pre-saved egmentation model")
    ...
    if run_semantic_training:
        ...
        if check_oxford_batch:
            view_batch(train_loader, ft_segmentation_model)


        # check_random_image(ft_segmentation_model, params)
        ...
        for epoch in range(ft_num_epochs):
            ...
            for its, (images, labels) in enumerate(train_loader):
                images, labels_one_hot = images.to(device), labels.to(device)

                # forward pass
                outputs = ft_segmentation_model(images)
                labels = torch.argmax(labels_one_hot, dim=1)
                # plt_img_lbl_out(images[0].cpu().permute(1,2,0), labels[0].cpu()/2, outputs[0].detach().cpu().permute(1,2,0))
                # print(outputs.detach())
                #outputs = outputs.reshape(ft_batch_size,3,-1)
                #labels = labels.reshape(ft_batch_size,-1)
                ...
                running_loss += loss.detach().cpu().item()
                if its % report_every == (report_every-1):    # print every report_every mini-batches
                    print('Epoch [%d / %d],  %d image minibatch [%4d / %4d], running loss: %.4f' % (epoch + 1, ft_num_epochs, ft_batch_size, its + 1, len(train_loader) ,running_loss / len(train_loader)))
                    # check_random_image(ft_segmentation_model, params)
            scheduler.step()
            epoch_end_time=time.time()
            losses.append(running_loss / len(train_loader))
            end_time_str = time.strftime("%H.%M", time.localtime(epoch_end_time))
            print(f"Epoch [{epoch + 1}/{ft_num_epochs}] completed at {end_time_str} taking {(epoch_end_time-epoch_start_time):.0f}s, Current LR {ft_optimizer.param_groups[0]['lr']:.7f}, Loss: {running_loss / len(train_loader):.4f}")
            #_= view_training(ft_segmentation_model, val_loader, False)
            #check_random_image(ft_segmentation_model, params)
    ...
    if check_semantic_segmentation:
        ...
        view_training(ft_segmentation_model, train_loader, True)
        #check_random_image(ft_segmentation_model, params)
